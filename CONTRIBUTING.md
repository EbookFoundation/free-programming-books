...
## In a nutshell
1. A link to easily download a book" is not always a link to a free book. Please only contribute to free content. Make sure it's free. We do not accept links to pages that require working email addresses to obtain books, but we welcome listings that request them.
...
- always prefer an `https` link over an `http` one -- as long as they are on the same domain and serve the same content
...
- if a link has an expired certificate/self-signed certificate/SSL issue of any other kind:
  ...
  2. *leave it* if no `http` version but the link still accessible through `https` by adding an exception to the browser or ignoring the warning
...
- if an email address or account setup is requested before the download is enabled, add language-appropriate notes in parentheses, e.g.: `(email address *requested*, not required)`
...
- If you specify more than one file, the results of the build is based on the result of the last file checked. You should be aware that you may get passing green builds due to this so be sure to inspect the build log at the end of the pull request by clicking on "Show all checks" -> "Details".
