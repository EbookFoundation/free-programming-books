_Read this in other languages: [中文](CONTRIBUTING-zh.md)._
...
-   if a link has an expired certificate/self-signed certificate/SSL issue of any other kind:
    ...
    2. _leave it_ if no `http` version but link still accessible through `https` by adding an exception to the browser or ignoring the warning
